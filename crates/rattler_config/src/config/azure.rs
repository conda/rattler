...
impl AzureOptionsMap {
    ...
    /// The configured hosts, in the order they were written.
...
mod tests {
    ...
    fn keys_are_normalized_the_same_way_lookups_are() {
        for (written, looked_up) in [
            ...
            // would need rather than what happened to be typed, and writing the
            // table back out produces a key that parses to the same host.
            assert_eq!(map.keys(), vec![looked_up.to_string()], "{written}");
