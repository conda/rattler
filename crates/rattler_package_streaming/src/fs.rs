...
///
/// If `cas_path` is provided, file contents are stored in the CAS and hardlinked
/// to the destination for deduplication. The CAS path and destination must be on
/// the same filesystem for hardlinking to work.
///
/// ```rust,no_run
/// # use std::path::Path;
...
pub fn extract_tar_bz2(
    archive: &Path,
    destination: &Path,
    cas_path: Option<&Path>,
) -> Result<ExtractResult, ExtractError> {
    let file = File::open(archive)?;
    crate::read::extract_tar_bz2(file, destination, cas_path)
}
...
pub fn extract_conda(
    ...
    let file = File::open(archive)?;
    crate::read::extract_conda_via_streaming(file, destination, cas_path)
}
...
pub fn extract(
    ...
    match CondaArchiveType::try_from(archive).ok_or(ExtractError::UnsupportedArchiveType)? {
        CondaArchiveType::TarBz2 => extract_tar_bz2(archive, destination, cas_path),
        CondaArchiveType::Conda => extract_conda(archive, destination, cas_path),
    }
}
