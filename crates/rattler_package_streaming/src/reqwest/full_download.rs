...
/// Stream the full package response and list files from the `info/` section.
pub async fn list_info_files_from_remote_full_download(
    client: &ClientWithMiddleware,
    url: &Url,
    ...
    match archive_type {
        CondaArchiveType::TarBz2 => {
            ...
            let decoder = BzDecoder::new(buf_reader);
            let mut archive = tokio_tar::Archive::new(decoder);
            list_files_in_tar_archive(&mut archive, Some(Path::new("info"))).await
        }
        CondaArchiveType::Conda => {
            let compat_reader = stream_reader.compat();
            let mut buf_reader = futures::io::BufReader::new(compat_reader);
            ...
            found.ok_or(ExtractError::MissingComponent)
        }
    }
}
