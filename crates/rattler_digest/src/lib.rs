...
//! - [`parse_digest_from_hex`]: Given a hex representation of a digest, parses
//!   it to bytes.
//! - [`HashingWriter`]: An object that wraps a writable object and implements
//!   [`Write`] and [`::tokio::io::AsyncWrite`]. It forwards the data to the
//!   wrapped object but also computes the hash of the content on the fly.
//!
...
/// to retrieve both the original `impl Write` object as well as the hash.
///
/// If the `tokio` feature is enabled this object also implements
/// [`::tokio::io::AsyncWrite`] which allows you to use it in an async context
/// as well.
...
impl<W: Write, D: Digest> Write for HashingWriter<W, D> {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        self.hasher
            .update(buf.get(..bytes).expect("safe because bytes <= buf"));
        Ok(bytes)
    }
...
///
/// If the `tokio` feature is enabled this object also implements
/// [`::tokio::io::AsyncRead`] which allows you to use it in an async context as
/// well.
...
impl<R: Read, D: Digest> Read for HashingReader<R, D> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        self.hasher.update(
            buf.get(..bytes_read)
                .expect("safe because bytes_read <= buf"),
        );
        Ok(bytes_read)
    }
}
...
mod test {
    ...
    fn test_compute_file_sha256(#[case] input: &str, #[case] expected_hash: &str) {
        // Write a known value to a temporary file and verify that the compute hash
