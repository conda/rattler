    ...
    fn try_from(value: CondaPackageDataModel<'a>) -> Result<Self, Self::Error> {
            ...
            .is_some_and(|name| ArchiveIdentifier::try_from_filename(name).is_some())
        {
            ...
            let (derived_arch, derived_platform) =
                derived_fields::derive_arch_and_platform(&subdir);
            ...
            // Source package - extract only the fields we need
    ...
    fn from(value: &'a CondaPackageData) -> Self {
        match value {
            CondaPackageData::Binary(binary_data) => {
                let package_record = &binary_data.package_record;
                let derived = LocationDerivedFields::new(&binary_data.location);
                let derived_build_number =
                    derived_fields::derive_build_number_from_build(&package_record.build)
                        .unwrap_or(0);
                ...
                let channel = binary_data.channel.as_ref();
                let file_name = Some(binary_data.file_name.as_str());
                ...
                Self {
                    location: binary_data.location.clone(),
                    ...
                    experimental_extra_depends: Cow::Borrowed(
                        &package_record.experimental_extra_depends,
                    ),
            ...
            CondaPackageData::Source(source_data) => {
                ...
                Self {
                    ...
                    channel: None,
                    file_name: None,
                    purls: Cow::Borrowed(&source_data.purls),
                    depends: Cow::Borrowed(&source_data.depends),
                    constrains: Cow::Borrowed(&source_data.constrains),
                    experimental_extra_depends: Cow::Borrowed(
                        &source_data.experimental_extra_depends,
                    ),
                    md5: None,
                    legacy_bz2_md5: None,
                    sha256: None,
                    size: Cow::Borrowed(&None),
                    legacy_bz2_size: Cow::Borrowed(&None),
                    timestamp: None,
                    features: Cow::Borrowed(&None),
                    track_features: Cow::Owned(Vec::new()),
                    license: Cow::Borrowed(&source_data.license),
                    license_family: Cow::Borrowed(&None),
                    python_site_packages_path: Cow::Borrowed(
                        &source_data.python_site_packages_path,
                    ),
                    input: source_data.input.as_ref().map(|input| InputHash {
                        hash: input.hash,
