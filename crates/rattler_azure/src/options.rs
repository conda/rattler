...
//! the ergonomic spelling and it mirrors `s3-options`. Internally each is an
//! enum, so no call site can mix up two unrelated booleans, and the meaning of a
//! value is legible without chasing the field name. The bridge is
//! `#[serde(from = "bool", into = "bool")]` plus a `From` impl each way.
...
///
/// Defaults to [`Scheme::Https`]. `Http` exists for local emulators such as
...
/// accounts. Serializes as the bool `path-style` in `azure-options`. Mirrors
/// `rattler_s3::S3AddressingStyle`.
#[derive(Default, Clone, Copy, Debug, PartialEq, Eq, Deserialize, Serialize)]
#[serde(from = "bool", into = "bool")]
pub enum Addressing {
    /// The account is the first label of the host, as in
    /// `<account>.blob.core.windows.net/<container>`. Requires a dotted host, so
    /// IP literals and single-label hosts cannot be addressed this way.
    #[default]
    HostStyle,
    ...
    fn toml_bools_map_to_enums() {
        ...
        assert_eq!(
            ...
            AzureEndpointOptions {
                ...
                scheme: Scheme::Http,
                path_style: Addressing::PathStyle,
            }
        );
        ...
        assert_eq!(empty.scheme, Scheme::Https);
        assert_eq!(empty.path_style, Addressing::HostStyle);
