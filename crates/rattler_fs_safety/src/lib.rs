...
pub use cap_std::fs::{
    Dir, DirBuilder, DirBuilderExt, File, OpenOptions, OpenOptionsExt, Permissions, PermissionsExt,
};
...
/// On Windows, cap-std surfaces reparse-point refusals through
/// `io::Error` codes that vary by configuration. The kernel
/// typically already returns an `ACCESS_DENIED`-style error which
/// `io::Error` maps to `PermissionDenied`, so let cap-std's error
/// pass through unchanged rather than guess at a specific code.
#[cfg(not(unix))]
...
/// a per-call private subdirectory of `dir` (on Unix, created
/// with mode `0o700` and stat-verified to belong to the current
/// effective uid), so no other process can populate or
/// substitute the tempfile. The final publish is an `fd`-anchored
/// cross-directory `renameat`, which does not follow symlinks at
/// the destination's final component; `name` is enforced
/// single-component so no destination-side intermediate
/// component is traversed either.
///
/// On Windows, `mode` is currently ignored (Windows has no
/// `chmod`-style permission word). The directory built by
/// [`TempDir::new_in`] is created with cap-std's defaults — the
/// Windows ACL model and the `Dir` handle's reparse-point-aware
/// open are what carry the safety guarantees there.
///
...
pub fn atomic_write_in(dir: &Dir, name: &OsStr, bytes: &[u8], mode: Option<u32>) -> io::Result<()> {
    ...
    #[cfg(unix)]
    builder.mode(0o700);
    let tmp = TempDir::new_in(dir, ".rattler-write-", &builder)?;

    // Create the payload directly with restrictive perms on Unix
    // (no `umask`-dependent window) and with `O_CREAT | O_EXCL`
    // on every platform. The file lives inside `tmp`, which on
    // Unix is `0o700` and stat-verified — a co-tenant cannot
    // substitute the payload before we rename it out.
    let mut opts = OpenOptions::new();
    opts.write(true).create_new(true);
    #[cfg(unix)]
    opts.mode(0o600);
    let payload_name = OsStr::new("payload");
    let mut payload = tmp.open_with(payload_name, &opts)?;
    ...
    // Atomically publish via fd-anchored cross-directory
    // `renameat`. The source path is anchored to our private
    // `tmp` capability and the destination to the caller's
    // `dir` capability; `name` is single-component, so there's
    // nothing to traverse on either side.
...
/// [`atomic_write_in`]: a private workspace where attacker-uncontrolled
/// tempfiles can be created and renamed out from. Compared to
/// [`cap_tempfile::TempDir`] it adds two things:
///
/// * **A caller-supplied [`DirBuilder`].** This is what lets the
///   helper request mode `0o700` atomically at `mkdirat` time on
///   Unix; the cap-tempfile flavor uses default mode (`0o777`
///   modulo `umask`, typically `0o755`), which leaves a permission
///   window during which co-tenants can populate the directory.
/// * **Stat-verification after open.** Between `mkdirat` and
///   `openat` a co-tenant with write access to the parent
///   directory could rename our subdirectory away and
///   substitute one they own. After `openat`, we re-stat the
///   opened fd and (on Unix) refuse to proceed unless its owner
///   uid matches the current effective uid. A different-uid
///   attacker cannot fake that: `mkdir` always sets the owner
///   to the caller's uid, and only `root` can `chown`.
///
...
impl TempDir {
    ...
    pub fn new_in(parent: &Dir, prefix: &str, builder: &DirBuilder) -> io::Result<Self> {
        ...
        for _ in 0..16 {
            ...
            match parent.create_dir_with(&name, builder) {
                Ok(()) => {
                    let dir = parent.open_dir(&name).inspect_err(|_| {
                        // We created the directory but couldn't
                        // open it. Best-effort path-based cleanup
                        // so we don't leak the entry. If a
                        // co-tenant substituted the entry before
                        // our `remove_dir`, the rmdir either
                        // succeeds (their dir was empty) or fails
                        // silently (we leak their dir, not ours).
                        let _ = parent.remove_dir(&name);
                    })?;
                    #[cfg(unix)]
                    Self::verify_unix(&dir).inspect_err(|_| {
    ...
    /// Stat-verify that the opened directory is still owned by
    /// the current effective uid. Catches the substitution race
    /// between `mkdirat` and `openat`.
    #[cfg(unix)]
    fn verify_unix(dir: &Dir) -> io::Result<()> {
