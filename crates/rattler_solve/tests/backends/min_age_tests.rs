...
/// Test that packages without timestamps are not affected by min_age.
pub fn solve_min_age_no_timestamp<T: SolverImpl + Default>() {
    let repo = vec![
        PackageBuilder::new("pkg-no-ts")
            .version("1.0")
            // No timestamp set - should not be filtered
            .build(),
    ...
    SolverCase::new("min_age with no timestamp")
        .repository(repo)
        .specs(["pkg-no-ts", "pkg-old"])
        .min_age(MinimumAgeConfig::new(min_age))
        // Both packages should be available:
        // - pkg-no-ts has no timestamp so it's not filtered
