...
use cache::{CacheHeaders, CacheState, Expiring};
...
impl From<reqwest::Error> for NotFoundError {
    fn from(err: reqwest::Error) -> Self {
...
/// handle file:/// urls
async fn run_exports_from_file(
    run_exports_url: Url,
    out_path: PathBuf,
    cache_state_path: PathBuf,
    lock_file: LockedFile,
) -> Result<Cache, FetchError> {
    // copy file from subdir_url to out_path
    if let Err(e) = tokio::fs::copy(&run_exports_url.to_file_path().unwrap(), &out_path).await {
        return if e.kind() == ErrorKind::NotFound {
            Err(FetchError::NotFound(NotFoundError::FileSystemError(e)))
        } else {
...
pub async fn fetch_data(
    ...
    let url = subdir_url.join(file_name).expect("file name is invalid");
    // Compute the cache key from the url
    let cache_key = crate::utils::url_to_cache_filename(&url);
    let run_exports_json_path = cache_path.join(format!("{cache_key}.json"));
    ...
    let lock_file = tokio::task::spawn_blocking(move || {
        LockedFile::open_rw(lock_file_path, "run exports cache")
    })
    .await?
    ...
    let cache_action = if subdir_url.scheme() == "file" {
        ...
        return run_exports_from_file(url, run_exports_json_path, cache_state_path, lock_file)
            .await;
    } else {
    ...
    let variant_availability =
        check_variant_availability(&client, &subdir_url, cache_state.as_ref(), file_name).await;
    ...
    let repo_data_url = if has_zst {
        subdir_url.join(&format!("{}.zst", file_name)).unwrap()
    } else if has_bz2 {
        subdir_url.join(&format!("{}.bz2", file_name)).unwrap()
    } else {
        url
    };
    ...
    let response = match request_builder.headers(headers).send().await {
        Ok(response) if response.status() == StatusCode::NOT_FOUND => {
            return Err(FetchError::NotFound(NotFoundError::from(
    ...
    let repo_data_destination_path = run_exports_json_path.clone();
...
#[cfg(test)]
mod test {
    use super::{fetch_data, Cache, CacheResult, DownloadProgress, Options};
    use crate::fetch::{FetchError, NotFoundError};
    ...
    pub async fn test_cache_works() {
        ...
        // Download the data from the channel with an empty cache.
        let cache_dir = TempDir::new().unwrap();
        let Cache { cache_result, .. } = fetch_data(
            server.url(),
            "run_exports.json",
            ClientWithMiddleware::from(Client::new()),
    ...
    pub async fn test_repodata_not_found() {
        ...
        assert!(matches!(
            result,
            Err(FetchError::NotFound(NotFoundError::FileSystemError(_)))
        ));
        ...
        assert!(matches!(
            result,
            Err(FetchError::NotFound(NotFoundError::HttpError(_)))
        ));
    }
}
