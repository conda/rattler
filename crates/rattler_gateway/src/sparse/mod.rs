//! This module provides the [`SparseRunExports`] which is a struct to enable only sparsely loading records
...
struct SparseRunExportsInner {
    ...
    #[borrows(memory_map)]
    #[covariant]
    run_exports: LazyRunExports<'this>,
}
...
impl SparseRunExports {
    ...
    pub fn new(
        ...
        Ok(SparseRunExports {
            inner: SparseRunExportsInnerTryBuilder {
                memory_map,
                run_exports_builder: |memory_map| serde_json::from_slice(memory_map.as_ref()),
            }
            .try_build()?,
            subdir: subdir.into(),
            channel,
    ...
    pub fn package_names(&self) -> impl Iterator<Item = &'_ str> + '_ {
        let run_exports = self.inner.borrow_run_exports();
        run_exports
            .packages
            .iter()
            .chain(run_exports.conda_packages.iter())
    ...
    pub fn load_records_recursive<'a>(
        repo_data: impl IntoIterator<Item = &'a SparseRunExports>,
        package_names: impl IntoIterator<Item = PackageName>,
        patch_function: Option<fn(&mut PackageRecord)>,
    ) -> io::Result<Vec<Vec<RepoDataRecord>>> {
        let run_exports: Vec<_> = repo_data.into_iter().collect();

        // Construct the result map
        let mut result: Vec<_> = (0..run_exports.len()).map(|_| Vec::new()).collect();
        ...
        while let Some(next_package) = pending.pop_front() {
            for (i, run_export) in run_exports.iter().enumerate() {
                let run_export_packages = run_export.inner.borrow_run_exports();
                let base_url = run_export_packages
                    .info
                    .as_ref()
                    .and_then(|i| i.base_url.as_deref());
...
/// Parse the records for the specified packages from the [`RawValue`]
fn parse_records<'i>(
...
pub async fn load_repo_data_recursively(
    ...
    let lazy_repo_data = stream::iter(repo_data_paths)
        .map(|(channel, subdir, path)| {
            ...
            tokio::task::spawn_blocking(move || {
                SparseRunExports::new(channel, subdir, path, patch_function)
            })
    ...
    SparseRunExports::load_records_recursive(&lazy_repo_data, package_names, patch_function)
}
