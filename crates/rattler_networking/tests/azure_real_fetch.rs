...
//!
//! Unlike `azure_azurite_fetch.rs` (shared-key vs. the emulator), this exercises
//! the AAD / token path of reqsign's `DefaultCredentialProvider` — i.e. the
//! credential `az login` provides — against a real `*.blob.core.windows.net`
//! endpoint. It is purely a read; it never writes.
//!
...
async fn azure_middleware_fetches_real_repodata() {
    ...
    let endpoint_url = std::env::var("AZURE_TEST_ENDPOINT")
        .ok()
        .map(|e| Url::parse(&e).expect("invalid AZURE_TEST_ENDPOINT"));
