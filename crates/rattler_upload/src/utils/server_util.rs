...
pub fn extract_prefix_info(url: &Url) -> Result<(Url, String), Box<dyn std::error::Error>> {
    let url_str = url.as_str();
    let prefix_pattern: Regex =
        Regex::new(r"^https?://(?:www\.)?prefix\.dev(?:/api/v1/upload/([^/]+)|/([^/]+))?$")
            .unwrap();
...
pub fn extract_anaconda_info(url: &Url) -> Result<(Url, Vec<String>), Box<dyn std::error::Error>> {
    let url_str = url.as_str();
    let anaconda_pattern =
        Regex::new(r"^(https?://(?:upload\.)?anaconda\.org)(?:/([^/]+))?").unwrap();
...
pub fn extract_s3_info(url: &Url) -> Result<(Url, Url, String), Box<dyn std::error::Error>> {
    ...
    if url_str.starts_with("s3://") {
        ...
        let s3_pattern =
            Regex::new(r"^(https?://([^.]+)\.s3(?:\.([^.]+))?\.amazonaws\.com)(?:/([^/]+))?")
                .unwrap();
        if let Some(captures) = s3_pattern.captures(url_str) {
