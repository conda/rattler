...
//! # Effective Walk Root
//!
//! When glob patterns contain relative path components like `..`, they may reference directories
//! outside the user's specified search root. For example, if you're searching from `/project/subdir`
//! with the pattern `../src/*.rs`, the pattern actually targets `/project/src/`.
//!
//! The **effective walk root** is the common ancestor directory from which all provided patterns
//! can be evaluated. This module calculates how many `..` segments need to be traversed to find
//! that common ancestor.
//!
//! # Rebasing
//!
//! **Rebasing** is the process of adjusting glob patterns so they work correctly relative to the
//! effective walk root. When patterns are rebased:
//!
//! - The `..` components are resolved by moving the walk root up the directory tree
//! - Any concrete path components between the `..` segments and the glob portion are spliced
//!   back into the pattern
//!
//! ## Example
//!
//! Given search root `/project/subdir` and patterns:
//! - `../src/*.rs` (targets `/project/src/`)
//! - `*.txt` (targets `/project/subdir/`)
//!
//! The effective walk root becomes `/project` (one level up), and patterns are rebased to:
//! - `src/*.rs`
//! - `subdir/*.txt`
//!
//! This allows a single directory walker to efficiently match all patterns from one starting point.
//!
//! # Global Exclusions
//!
//! Negated patterns starting with `**/` (e.g., `!**/build.rs`) are treated as global exclusions
//! and are **not** rebased. This ensures wildcard exclusions continue to apply everywhere,
...
pub enum WalkRootsError {
    ...
    #[error("expected prefix '{prefix}' to be relative, it is absolute")]
...
impl WalkRoot {
    ...
    pub fn rebase(&self, root: &Path) -> Result<RebasedGlobs, WalkRootsError> {
        ...
        for spec in &self.specs {
            ...
            let distance_from_prefix = self.max_parent_dirs.saturating_sub(spec.parent_dirs);

            // Create the glob prefix
            let mut components = Vec::new();
            components.extend(popped.iter().take(distance_from_prefix).cloned());
...
mod tests {
    ...
    use super::{normalize_relative, split_path_and_glob, WalkRoot};
