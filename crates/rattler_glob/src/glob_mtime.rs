...
/// Errors that can occur when computing glob modification times.
#[derive(Error, Debug)]
pub enum GlobModificationTimeError {
    /// Failed to retrieve the modification time for a matched file.
...
impl GlobModificationTime {
    /// Returns the modification time if files were found, or `None` if no files matched.
    ...
    /// Returns the path of the file with the newest modification time, or `None` if no files matched.
    pub fn file(&self) -> Option<&Path> {
        match self {
            Self::NoMatches => None,
            Self::MatchesFound {
                designated_file, ..
            } => Some(designated_file),
        }
    }

    /// Returns `true` if any files matched the glob patterns.
    ...
    /// matching the provided glob patterns, and returns the newest modification time
    /// along with the path of that file.
    ///
    /// # Arguments
    ...
    ///
    /// # Returns
    /// A `GlobModificationTime` indicating whether files were found and their newest
    /// modification time, or an error if the operation failed.
    ///
    /// # Example
    /// ```no_run
    /// use rattler_glob::GlobModificationTime;
    /// use std::path::Path;
    ///
    /// let mtime = GlobModificationTime::from_patterns(
    ///     Path::new("/my/project"),
    ///     ["src/**/*.rs"],
    /// ).unwrap();
    ///
    /// if let Some(time) = mtime.modified_at() {
    ///     println!("Newest file modified at: {:?}", time);
    /// }
    /// ```
    ...
    pub fn from_patterns_ignore<'a>(
        ...
        for entry in entries {
            ...
            if latest.is_some_and(|cur| cur >= modified) {
                continue;
            }
