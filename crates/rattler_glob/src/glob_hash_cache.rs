...
impl GlobHashKey {
    ...
    ///
    /// If the provided `root` is a file, its parent directory will be used instead.
    /// If the file has no parent (e.g., a root path like `/` or `C:\`), the path
    /// is used unchanged.
                ...
                match entry.get() {
                    HashCacheEntry::Pending(weak_tx) => {
                        ...
                        subscriber
                            .recv()
                            .await
                            .map_err(|_recv_error| GlobHashError::Cancelled)
                    }
