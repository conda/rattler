...
pub fn run_in_environment(
    ...
    for (k, v) in env_vars.iter() {
        shell_script.set_env_var(k, v).map_err(ActivationError::from)?;
    }
    ...
    shell_script.run_script(script).map_err(ActivationError::from)?;
    let file = tempfile::Builder::new()
        .suffix(&format!(".{}", shell.extension()))
        .tempfile()?;
    fs_err::write(file.path(), shell_script.contents().map_err(ActivationError::from)?)?;
