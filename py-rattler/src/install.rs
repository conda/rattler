...
use rattler::{
    install::{
        InstallDriver, InstallOptions, PythonInfo, empty_trash, link_package, unlink_package,
    },
};
...
pub fn py_link_package<'a>(
    ...
    future_into_py(py, async move {
        ...
        if let Some(limit) = io_concurrency_limit {
            driver_builder = driver_builder.with_io_concurrency_semaphore(
                Arc::new(Semaphore::new(limit))
            );
        }
        
        // Configure whether to execute link scripts
        driver_builder = driver_builder.execute_link_scripts(execute_link_scripts);
        
        // Add prefix records if specified
