...
**Solving got a lot faster.** resolvo went from 0.10.3 to 0.12.0 over this release, with a reworked clause encoding and decision queue. On the `rattler_solve` benchmarks that takes 12% to 41% off the solve time, and the harder the problem the bigger the win: `quetz` drops from 413 ms to 242 ms, `tensorflow` from 283 ms to 175 ms. Nothing to change on your side.

**Read a single file out of a package without downloading it.** `PackageArchive` opens a `.conda` archive over HTTP range requests and pulls out only the entries you ask for. Inspecting metadata no longer costs a full download. Works on local paths too.
...
- `PackageArchive` for sparse reads from remote and local packages, with `read_file`, `read_files`, `index_json` and `paths_json` in [#2632](https://github.com/conda/rattler/pull/2632)
...
- Render build-only match specs with a `*` version placeholder so `str(MatchSpec)` round-trips: `foo py39h123_0` becomes `foo * py39h123_0` in [#2610](https://github.com/conda/rattler/pull/2610)
...
- Bump `resolvo` from 0.10.3 to 0.12.0, reworking clause encoding and the decision queue ([resolvo#236](https://github.com/prefix-dev/resolvo/pull/236), [#238](https://github.com/prefix-dev/resolvo/pull/238), [#245](https://github.com/prefix-dev/resolvo/pull/245), [#246](https://github.com/prefix-dev/resolvo/pull/246)): solves run 12% to 41% faster on the `rattler_solve` benchmarks in [#2520](https://github.com/conda/rattler/pull/2520) and [#2609](https://github.com/conda/rattler/pull/2609)
