...
async def test_who_needs_by_name(gateway: Gateway, dummy_channel: Channel) -> None:
    ...
    # `constrains`.
    dependents = await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], "bors")
    ...
    by_name = await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], PackageName("bors"))
...
async def test_who_needs_by_record(gateway: Gateway, dummy_channel: Channel) -> None:
    ...
    records = await gateway.query([dummy_channel], [PlatformName.LINUX_64], ["bors"], recursive=False)
    by_version = {str(record.version): record for record in records[0]}

    dependents = await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], by_version["1.1"])
    assert {name for name, _, _ in summary(dependents)} == {"foo", "foobar"}

    assert await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], by_version["2.1"]) == []
...
async def test_who_needs_virtual_package(gateway: Gateway, dummy_channel: Channel) -> None:
    ...
    dependents = await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], "__cuda")
    ...
    assert summary(await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], cuda)) == summary(dependents)
    old_cuda = GenericVirtualPackage(PackageName("__cuda"), Version("11.8"), "0")
    assert await gateway.who_needs([dummy_channel], [PlatformName.LINUX_64], old_cuda) == []
...
async def test_who_needs_extra_depends(gateway: Gateway, test_data_dir: str) -> None:
    ...
    dependents = await gateway.who_needs([channel], [PlatformName.NOARCH], "bar")
    assert summary(dependents) == [
    ...
    records = await gateway.query([channel], [PlatformName.NOARCH], ["bar"], recursive=False)
    bar_1 = next(record for record in records[0] if str(record.version) == "1")
    assert extras(await gateway.who_needs([channel], [PlatformName.NOARCH], bar_1)) == ["extra1", "with-bar"]
...
async def test_who_needs_multi_platform(gateway: Gateway, conda_forge_channel: Channel) -> None:
    ...
    combined = await gateway.who_needs(
        [conda_forge_channel], [PlatformName.LINUX_64, PlatformName.NOARCH, PlatformName.LINUX_64], "python_abi"
    )
    assert combined
    assert all(
    ...
    per_platform = [
        key(dependent)
        for platform in (Platform(PlatformName.LINUX_64), Platform(PlatformName.NOARCH))
